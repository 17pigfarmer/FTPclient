...
void FTPclient::on_query_button_clicked() {
	...
	//log in
	int ret = ftp.LogIn(control_socket, ui.username_text->text().toStdString(), ui.password_text->text().toStdString());
	switch(ret) {
		case -2:
	...
	//get filelist
	string filelist;
	ftp.ListFile(control_socket,filelist);
...
void FTPclient::on_download_button_clicked()
{
	...
	ftp.DownloadFile(control_socket, filename, ui.dir_text->text().toStdString());
}
...
void FTPclient::on_quit_button_clicked()
{
	...
	ftp.CloseSocket(control_socket);
	ui.files->clear();
	ui.files->setDragDropMode(QAbstractItemView::NoDragDrop);
